...
class RectTool: AnnotationTool {
    var name: String = "矩形"
    var iconName: String = "rect-tool"
    var color: NSColor = NSColor(red: 1.0, green: 0.23, blue: 0.19, alpha: 1.0)
    var lineWidth: CGFloat = 3.0
    
    private var startPoint: CGPoint?
    private var currentRect: CGRect?
    private var currentElement: RectElement?
    ...
    func drag(to point: CGPoint, in canvas: AnnotationCanvas) {
        ...
        currentRect = rect
        currentElement = RectElement(rect: rect, color: color, lineWidth: lineWidth)
